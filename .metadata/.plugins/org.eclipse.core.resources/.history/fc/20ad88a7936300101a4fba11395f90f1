...
public class StripeService {
    ...
    public String createCustomerPortalSession(String customerId, String returnUrl) {
        ...
        try {
            ...
            return session.getUrl();  // ポータルのURL
        } catch (StripeException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public void cancelSubscription(User user) {
        try {
            ...
            if (user.getRole() != null && user.getRole().getName().equals("ROLE_PAID")) {
                Role generalRole = roleRepository.findByName("ROLE_GENERAL");
                user.setRole(generalRole);
            }
            userRepository.save(user);

            System.out.println("★ ユーザー情報更新済み");
    ...
    public void handleSuccess(HttpServletRequest request, User user) {
        try {
            ...
            if (sessionId != null) {
                ...
                if (paidRole != null && (user.getRole() == null || !user.getRole().getName().equals("ROLE_PAID"))) {
